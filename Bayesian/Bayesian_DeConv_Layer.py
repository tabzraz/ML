...
class Bayesian_DeConv:
    ...
    def __init__(self,
        ...
        with tf.name_scope(name):
            with tf.name_scope("Weights"):
                qw_mean = tf.Variable(initial_value=np.ones(shape=(filter_height, filter_width, filters, input_channels)) * self.qw_mean_initial, dtype=tf.float32, name="Filter_weight_means")
                qw_p = tf.Variable(initial_value=np.ones(shape=(filter_height, filter_width, filters, input_channels)) * self.qw_logstd_initial, dtype=tf.float32, name="Filter_weight_lodstds")
    ...
    def normal_sample(self, input_tensor, use_mean=False):
        epsilon_w = tf.random_normal(shape=(self.filter_height, self.filter_width, self.filters, self.input_channels))
        ...
        conved = tf.nn.conv2d_transpose(input_tensor, W, output_shape=output_shape, strides=[1, self.filter_stride, self.filter_stride, 1], padding="VALID")
    ...
    def local_reparam_sample(self, input_tensor):
        ...
        gamma = tf.nn.conv2d_transpose(input_tensor, self.qw_mean, output_shape=output_shape, strides=[1, self.filter_stride, self.filter_stride, 1], padding="VALID")
        gamma = tf.nn.bias_add(gamma, self.qb_mean)
        # print(gamma)
        delta = tf.nn.conv2d_transpose(tf.square(input_tensor), tf.square(self.qw_sigma), output_shape=output_shape, strides=[1, self.filter_stride, self.filter_stride, 1], padding="VALID")
